...
namespace IngameScript
{
    partial class Program
    {
        ...
        // ScreenMenu
        //----------------------------------------------------------------------
        public class ScreenMenu : IScreenSpriteProvider
        {
            ScreenSprite back;
            ScreenSprite title;
            ...
            float _fontSize = 1f;
            public ScreenMenu(string title, float width, ScreenActionBar actionBar, float fontSize = 1f)
            {
                _width = width;
            ...
            {
                menuItems.Add(new ScreenMenuItem(label, _width, _fontSize));
            }
            public void AddLabel(string label, string defaultValue)
            {
                //GridInfo.Echo("AddLabel: " + label + ": " + defaultValue);
                menuItems.Add(new ScreenMenuItem(label, _width, defaultValue, _fontSize));
            }
            //
            ...
            {
                menuItems.Add(new ScreenMenuItem(label, varName, _width, defaultValue, _fontSize));
            }
            ...
            {
                Vector2 position = new Vector2(ItemIndent / 2, Height / -2);
                if (back != null)
                {
                ...
                position.Y += 30 * headerHeight;
                position.X = ItemIndent;
                int i = 0;
            ...
            // remove from screen
            //
            public virtual void RemoveToScreen(Screen screen)
            {
            ...
            {
                if (argument.ToLower().StartsWith("hotbar"))
                {
                    // handle button press
